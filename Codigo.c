...
static long timespec_diff_ns(const struct timespec *end,
...
/////////////////////// Simulação de carga de 5ms (SORT da M2) ///////////////////////
static void busy_5ms(void) {
    struct timespec req = {0};
    req.tv_nsec = 5 * NS_PER_MS;
    nanosleep(&req, NULL);
}
...
void *server_thread(void *arg) {
    ...
    while (1) {
        ...
        while (elapsed < Cs) {
            ...
            free(j); // libera nó da fila

            if (elapsed >= Cs) {
                printf("[SERVIDOR] Orçamento Cs esgotado no período!\n");
                break;
            }
        }
...
void *periodic_task(void *arg) {
    ...
    while (1) {
        ...
        printf("[%s] executando\n", p->name);
        busy_5ms();  // 5ms de carga
        clock_nanosleep(CLOCK_MONOTONIC, TIMER_ABSTIME, &next_release, NULL);
    }
    return NULL;
}

/////////////////////// GERADOR DE EVENTOS SORT (equiv. T0 da M2) ///////////////////////
void *sort_generator(void *arg) {
    (void)arg;
    int counter = 0;
    while (1) {
        // sensor T0 disparava de tempos imprevisíveis → simular 40–120ms
        int delay_ms = 40 + rand() % 80;
        struct timespec req = {0};
        req.tv_nsec = delay_ms * NS_PER_MS;
        nanosleep(&req, NULL);

        int *id = malloc(sizeof(int));
        *id = ++counter;

        printf("[GEN] Evento SORT gerado id=%d\n", *id);
        enqueue_job(sort_job, id);
    }
    return NULL;
}

/////////////////////// CRIA THREADS RT ///////////////////////
static void create_rt_thread(pthread_t *th,
                             void *(*func)(void*),
                             void *arg,
                             int priority)
{
...
int main(void) {
    ...
    server_params_t server_params = {
        ...
        .budget_ns = 20 * NS_PER_MS    // Cs = 20ms
    };
    pthread_t th_server;
    create_rt_thread(&th_server, server_thread, &server_params, 60);

    // ----------------- Tarefas periódicas -----------------
    periodic_task_params_t tparams[4] = {
        {"T1", 40 * NS_PER_MS},
        {"T2", 80 * NS_PER_MS},
        {"T3", 120 * NS_PER_MS},
        {"T4", 160 * NS_PER_MS}
    ...
    // ----------------- Gerador de eventos SORT -----------------
    pthread_t th_gen;
    pthread_create(&th_gen, NULL, sort_generator, NULL);
